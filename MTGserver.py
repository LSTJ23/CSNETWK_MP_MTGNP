...
class MTGNPServer:
    ...
    def send_game_state_update(self, player_info):
        ...
        try:
            self.send_pdu_verbose(player_info["socket"], state_pdu)
    ...
    def broadcast(self, pdu_template: dict):
        for p in self.players:
            ...
            try:
                self.send_pdu_verbose(p["socket"], pdu)
            except Exception:
                pass
    ...
    def grant_priority(self, player_id: str, timeout_ms: int = DEFAULT_PRIORITY_TIMEOUT_MS):
        ...
        if target_conn:
            try:
                self.send_pdu_verbose(target_conn["socket"], pdu)
            except ConnectionResetError:
    ...
    def handle_client(self, player_info: dict):
        ...
        try:
            while True:
                try:
                    pdu = recv_pdu(sock)
                    self.log_verbose(f"INCOMING PDU [{pid}]: {json.dumps(pdu)}")
...
if __name__ == "__main__":
    ...
    server = MTGNPServer(port=DEFAULT_PORT, verbose=args.verbose)
    server.start()